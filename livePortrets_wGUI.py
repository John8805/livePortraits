...
def createControlWindow(controlWindowName, imageTrackbarName, videoTrackbarName, processTrackbarName):
    ...
    cv2.createTrackbar("Images", controlWindowName,
                       imageIndex, maximageIndex,  updateImageFN)
    # Trackbar to select the video
    cv2.createTrackbar("Videos", controlWindowName,
                       videoIndex, maxvideoIndex,  updateVideoFN)
    # Trackbar to start/stop the processing
    cv2.createTrackbar("Process", controlWindowName,
                       OnOFF, maxOnOFF, runProcess)
    ...
    text_size, _ = cv2.getTextSize(
        imageTrackbarName, font, font_scale, thickness)
...
def LivePortrets():
    ...
    if im_fn == '':
        ...
        if ret == True:
            ...
            # scale the frame to have a 600 pixel height
            scaleY = 600./frame.shape[0]
            frame = cv2.resize(src=frame, dsize=None, fx=scaleY,
        ...
        while(OnOFF == 1):
            ...
            if ret == True:
                ...
                scaleY = 600./frame.shape[0]
                frame = cv2.resize(
                    src=frame, dsize=None, fx=scaleY, fy=scaleY, interpolation=cv2.INTER_LINEAR)
                cv2.imshow("Live Portrets", frame)
    ...
    # if user selected one of the images
        ...
        if im is None:
            ...
            im = cv2.resize(src=im, dsize=None, fx=scaleY,
                            fy=scaleY, interpolation=cv2.INTER_LINEAR)
...
def runLivePortrets(im_fn, im, video_fn, cap):
    ...
    (time_video, length_video, fps, frame_width,
     frame_height) = getVideoParameters(cap)
    ...
    while(True):
        ...
        if ret == False:
            ...
            if isFirstFrame:
                ...
                landmarks_frame_init = landmarks2numpy(
                    landmarkDetector(frame, newRect_frame))
            ...
            frame_aligned = np.zeros(
                (im_height, im_width, im_channels), dtype=im.dtype)
            frame_aligned = cv2.warpAffine(frame, tform, (im_width, im_height))

            # Change the landmarks locations
            landmarks_frame = np.reshape(
                landmarks_frame_init, (landmarks_frame_init.shape[0], 1, landmarks_frame_init.shape[1]))
            landmarks_frame = cv2.transform(landmarks_frame, tform)
            landmarks_frame = np.reshape(
                landmarks_frame, (landmarks_frame_init.shape[0], landmarks_frame_init.shape[1]))
            ...
            if isFirstFrame:
                ...
                landmarks_frame = np.concatenate(
                    (landmarks_frame, np.zeros((numCP-68, 2))), axis=0)
            ...
            if (isFirstFrame == True):
                [pointsPrev.append((p[0], p[1]))
                 for p in landmarks_frame[68:, :]]
                [pointsDetectedPrev.append((p[0], p[1]))
                 for p in landmarks_frame[68:, :]]
            ...
            [pointsDetectedCur.append((p[0], p[1]))
             for p in landmarks_frame[68:, :]]
            ...
            lk_params = dict(winSize=(s, s), maxLevel=5, criteria=(
                cv2.TERM_CRITERIA_COUNT | cv2.TERM_CRITERIA_EPS, 20, 0.03))
            pointsArr, status, err = cv2.calcOpticalFlowPyrLK(
                imGrayPrev, imGray, pointsPrevArr, pointsArr, **lk_params)
            sigma = 100
            ...
            if isFirstFrame:
                ...
                srcPoints = insertBoundaryPoints(
                    im_width, im_height, srcPoints)

                lip_height = getLipHeight(landmarks_im)
                (_, _, maskInnerLips0, _) = teethMaskCreate(
                    im_height, im_width, srcPoints)
                mouth_area0 = maskInnerLips0.sum()/255

                # get source location on the first frame
                srcPoints_frame = landmarks_frame.copy()
                srcPoints_frame = insertBoundaryPoints(
                    im_width, im_height, srcPoints_frame)
            ...
            dstPoints_frame = insertBoundaryPoints(
                im_width, im_height, dstPoints_frame)
            ...
            (maskAllLips, hullOuterLipsIndex, maskInnerLips,
             hullInnerLipsIndex) = teethMaskCreate(im_height, im_width, dstPoints)
            ...
            maskInnerLips = cv2.GaussianBlur(
                np.stack((maskInnerLips, maskInnerLips, maskInnerLips), axis=2), (3, 3), 10)
...
createControlWindow(controlWindowName, imageTrackbarName,
                    videoTrackbarName, processTrackbarName)
